...
# Merge the sets
trainSet <- cbind(subjectrain,xtrain,ytrain)
testSet  <- cbind(subjectest,xtest,ytest)
...
data_grouped_with_act<-merge(data_grouped,activitylabs,by='IdActivity',all.x=TRUE)
# Arrange the columns in the data set
data_grouped_arrange<-data_grouped_with_act[,c(2,69,3:68)] 

# Create the txt file to be submitted
write.table(data_grouped_arrange, file = "./TidyData.txt",row.name=FALSE)
